...
try {
	...
	if (!$row) {
		throw new AppLevelException('Ссылка не&nbsp;верна или устарела.<br/>Возможно файл был удалён.<br/><br/><a href="'.$home_link.'">Перейти на главную страницу</a>');
	}
...
$preview_link_small = '';
if ($row['height'] > $pic_image_small_height && $preview_size != IMAGE_SIZE_SMALL) {
	$preview_link_small = '<li class="bigText"><a href="'.ami_link('links_image_owner', array($key_id, $key_delete, IMAGE_SIZE_SMALL)).'" title="Маленькая картинка"><img src="/img/gtk-zoom-out.png"/></a></li>';
}

$preview_link_middle = '';
if ($row['height'] > $pic_image_medium_height && $preview_size != IMAGE_SIZE_MIDDLE) {
	$preview_link_middle = '<li class="bigText"><a href="'.ami_link('links_image_owner', array($key_id, $key_delete, IMAGE_SIZE_MIDDLE)).'" title="Большая картинка"><img src="/img/gtk-zoom-in.png"/></a></li>';
}

//
$preview_link_preview = ami_link('links_image_owner', array($key_id, $key_delete, IMAGE_SIZE_PREVIEW));
//
...
ami_printPage($out, 'view_page_'.$preview_size);
exit();
